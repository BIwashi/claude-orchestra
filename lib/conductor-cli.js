...
${cyan('Commands:')}
  ${green('setup')}                     Check deps, render tracks, configure & start
...
function inferCurrentSectionIndex(sections) {
  // Prefer status.json (written by the running conductor) over log parsing
  const statusPath = join(ORCHESTRA_DIR, 'status.json');
  if (existsSync(statusPath)) {
    try {
      const status = JSON.parse(readFileSync(statusPath, 'utf-8'));
      if (status.sectionIndex != null && status.sectionIndex < sections.length) {
        return status.sectionIndex;
      }
    } catch {
