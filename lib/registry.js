...
function getOrchestraDir() {
  return process.env.CLAUDE_ORCHESTRA_DIR || join(process.env.HOME, '.claude-orchestra');
}

function getRegistryPath() {
  return join(getOrchestraDir(), 'registry.json');
}
...
class SessionRegistry {
  ...
  register(sessionId) {
    ...
    const usedInstruments = new Set([...this.sessions.values()].map((s) => s.instrumentId));
    const available =
      this.instruments.find((i) => !usedInstruments.has(i.id)) ||
...
export { SessionRegistry, getOrchestraDir, getRegistryPath };
