...
function renderTrack(scriptDir, dataDir, trackName, outputDir, soundfont) {
  const midiPath = join(dataDir, 'tracks', trackName, 'source.mid');
  const configPath = join(dataDir, 'tracks', trackName, 'track-config.json');
  const args = [
  ...
  if (existsSync(configPath)) {
    ...
    args.push('--name', trackName, '--timestamps', '0:00,0:30,1:00,1:30', '--events', '8');
  }
...
export async function setup() {
  ...
  if (hasFluidsynth && soundfont && existsSync(midiPath)) {
    ...
    if (existsSync(manifestPath)) {
      ...
      const success = renderTrack(scriptDir, dataDir, defaultTrack, trackOutputDir, soundfont);
      if (!success) {
