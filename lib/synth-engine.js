 ...
 */
class SynthEngine extends BaseEngine {
  constructor(config = {}) {
    super(config);
  ...
  handleToolEvent(event, instrument, sessionCount) {
    ...
    if (params) {
      this.player.playNote(params.instrumentId, params.noteName, this.applyVolume(params.volume));
    }
  }

  handleSessionJoin(instrument, count) {
    this.player.playArpeggio(
      instrument.id,
      ARPEGGIOS.join,
      this.applyVolume(volumeForSessions(count)),
    );
  }
  ...
  handleSessionLeave(instrument, count) {
    this.player.playArpeggio(
      instrument.id,
      ARPEGGIOS.leave,
      this.applyVolume(volumeForSessions(Math.max(1, count))),
    );
  }
  ...
  handleCompact(sessions) {
    ...
    for (let i = 0; i < sessions.length && i < notes.length; i++) {
      this.player.playNote(sessions[i].instrument.id, notes[i], this.applyVolume(0.6));
    }
  }
  ...
  handleIdle(sessions, chordIndex) {
    ...
    for (let i = 0; i < sessions.length && i < chord.length; i++) {
      ...
      this.player.playNote(inst.id, noteName, this.applyVolume(0.15));
    }
  }
