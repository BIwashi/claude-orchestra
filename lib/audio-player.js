 ...
 * Each channel manages its own afplay subprocess queue.
 */
class VoiceChannel {
  ...
  play(noteName, volume = 0.5) {
    ...
    // afplay with volume control (-v flag, 0-1 range mapped to afplay's scale)
    const afplayVolume = Math.max(0, Math.min(1, volume));
    this.currentProcess = spawn('afplay', ['-v', String(afplayVolume), wavPath], {
      stdio: 'ignore',
      detached: false,
    });
