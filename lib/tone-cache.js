...
import { getFrequency, NOTES, CHROMATIC } from './music-theory.js';

const CACHE_DIR = join(process.env.HOME, '.claude-orchestra', 'cache');
...
function generateTone(instrument, noteName, frequency, durationSec) {
  ...
  const harmonicFilters = instrument.harmonics.map((h, i) => {
...
export { generateAllTones, getTonePath, CACHE_DIR };
