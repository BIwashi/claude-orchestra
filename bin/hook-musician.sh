...
#
# MUST NOT block or fail — Claude Code waits for hooks to complete.
# All writes are best-effort; errors are silently swallowed.
...
# Timestamp for unique filename (nanoseconds where available)
...
echo "$INPUT" > "$TMP_FILE" 2>/dev/null && mv "$TMP_FILE" "$EVENT_FILE" 2>/dev/null
...
if [ -f "$STATUS_FILE" ]; then
  ...
  VOLUME=$(echo "$STATUS" | grep -o '"volume":[0-9.]*' | head -1 | cut -d: -f2)
  if [ -n "$TRACK" ]; then
    VOL_PCT=$(echo "$VOLUME" | awk '{printf "%d", $1 * 100}' 2>/dev/null || echo "?")
    echo "🎵 ${TRACK} — ${SECTION:-idle} | ${SESSIONS:-0} sessions | vol ${VOL_PCT}%"
  fi
fi
